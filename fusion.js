/* key_url = "/content/dam/bc1/schools/law/js/fusion/json/key.json"

async function fetchJSONFile(path, callback) {
...
async function assignAPIKey(){

}
var API_key;

fetchJSONFile(key_url, function(data){
...
function searchMenu() {
	...
	//always show all results on small displays - makes load too slow
	/*
	if ($('#courses').css("float") == "left"){
		var tables = $('.dataTable').DataTable();
		tables.page.len(9999).draw();
	}
	*/
	...
	// if the user searches less than 3 characters with pubs set to show all, 
	// change the pubs table to show fewer
...
//Create drop down menu from stored file of display name
...
var publicationsTable = $('#publicationsData').DataTable( {
	...
	"order": [[0,'desc'],[7,'asc'], [2, 'asc'], [1, 'asc']],
		...
		*/
		...
		/*
		{"year":2004,
		"title":"The Purer Fountains",
		"book":0,
		"publication":"Francis Bacon and the Refiguring of Early Modern Thought: Essays to Commemorate 'The Advancement of Learning' (1605-2005)",
		"name":"Daniel R. Coquillette",
		"disciplines":"Legal Education; Legal History",
		"fusionareas":"Legal History & Jurisprudence;",
		"URL":"http://lawdigitalcommons.bc.edu/lsfp/1",
		"priority":3}
		*/
...
function smallDraw() {//adds interaction after drawing the smallWindow Table

  //Groupings for types of faculty


  //Controls for each row of courses
  //Add control to rows with descriptions
...
//controls for each row of experiential
//Add control to rows with descriptions
...
//end of smallDraw function

	function smallWindow() { //This function is run in the drawCallback of each table
	if ($('#courses').css("float") == "left"){
    //reload the page if it is resized wider than 700px - that's the break for mobile behavior, and the page doesn't work well when resized across that breakline
    ...
    //clicking inthe table's div selects that table. Only one is displayed at a time. The active table is copied to a separate div in order to get the needed display
    $('.fusionTable')
    .unbind()
    .click(function() {
